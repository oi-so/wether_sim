...
9 km・3 km・1 kmの全領域で動画と地図付き3D/2D HTMLを出力します。既存結果の更新は `./scripts/animate_weather_case.sh CASE --force`（WRF再計算なし）。自由な回転・中心移動、滑らかなメッシュ、地表13項目、白い雲・レーダー風の雨色に対応しました。全3領域を接続する統合HTML、独立した地図ON/OFF、地形OFF時の0 m床、拡大できる風矢印にも対応しています。[操作と出力先](docs/animation_3d.md) を参照してください。

最新の[昼・夜・9/8〜9予測の評価](docs/evaluation_20260909.md)では、夜間気温の改善と、過剰な湿度・降水および予測の時間ずれを確認しています。
...
- `analysis/precipitation_animation.mp4`: 実出力間隔から換算した平均降水強度（mm/h）
- `analysis/precipitation_interval_animation.mp4`: 実出力間隔ごとの降水量（mm）
- `analysis/d02/`・`analysis/d01/`: 3 km・9 kmの同名動画と `atmosphere_3d.html`。d03は従来どおり `analysis/` 直下
